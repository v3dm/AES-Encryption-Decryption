...
def create_blob(ciphertext_b64: str, filename: Optional[str] = None, note: Optional[str] = None, owner: Optional[str] = None):
    db = SessionLocal()
    try:
        blob = EncryptedBlob(
            ciphertext_b64=ciphertext_b64,
...
def list_blobs(limit: int = 50) -> List[EncryptedBlob]:
    db = SessionLocal()
    try:
        return db.query(EncryptedBlob).order_by(EncryptedBlob.created_at.desc()).limit(limit).all()
    finally:
